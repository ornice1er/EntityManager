...
@extends('layouts.master')
 ...
 @section('content')
<a href="{{route('category-unities.create')}}" class="btn btn-primary float-right mr-2">Ajouter</a>                                                    
 <table class="table">
  <thead>
    <tr>
      <th scope="col">#</th>
  ...
  <tbody
  @foreach($category_unities as $category_unity)
    <tr>
      <th scope="row">{{$loop->index+1}}</th>
      <td>{{$category_unity->label}}</td>
      <td class="d-flex justify-content-end">
          <a href="{{route('category-unities.show',$category_unity->id)}}" class="btn btn-sm btn-info">Consulter</a>
